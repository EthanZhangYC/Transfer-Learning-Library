...
def main(args: argparse.Namespace):
    ...
    if 'cat_samedim' in args.nbr_mode:
        ...
        if args.nbr_label_mode == 'separate_input':
            ...
            classifier_features_dim = input_dim=64*2
    ...
    for epoch in range(args.epochs):
        # train for one epoch
        proto_s = train(train_source_iter, train_target_iter, G, F1, jmmd_loss, optimizer_g, optimizer_f, epoch, args, multihead_attn, \
        ...
        results = validate(val_loader, G, F1, attn_net, args, multihead_attn, \
...
def train(train_src_iter: ForeverDataIterator, train_tgt_iter: ForeverDataIterator, G: ImageClassifier, F1,
          jmmd_loss: JointMultipleKernelMaximumMeanDiscrepancy, optimizer_g: Adam, optimizer_f: Adam,
          epoch: int, args: argparse.Namespace, multihead_attn, nbr_label_encoder, bert_model, dim_converter, bert_learnable_tokens, enc_tokenizer, bert_learnable_tokens_class, proto_s, source_loader):
    ...
    for i in range(args.iters_per_epoch):
        ...
        cls_loss = F.cross_entropy(y1_s, labels_s)
        transfer_loss = jmmd_loss(
            (g_s, F.softmax(y1_s, dim=1)),
            (g_t, F.softmax(y1_t, dim=1))
        )
        ...
        trans_losses.update(transfer_loss.item(), bs)
...
def validate(val_loader: DataLoader, G: nn.Module, F1: ImageClassifierHead, attn_net:nn.Module, args: argparse.Namespace, multihead_attn, nbr_label_encoder, bert_model, dim_converter, bert_learnable_tokens, enc_tokenizer, bert_learnable_tokens_class) -> Tuple[float, float]:
